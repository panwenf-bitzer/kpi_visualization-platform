<!--
 * @FileDescription: child component for director sc throughput time daily
 * @Author: Pan,Wenfang
 * @Date: 2022/01/04
 ...
 * @LastEditTime: 2022/02/28
 -->
<template>
<div id="MD_sc_daily" :style="{height:height,width:width}" />
</template>
<script>
...
import {fetch_md_throughput_alu, fetch_md_throughput_sc} from "../../api/Director_dashboard";
export default {
  name: "MD_sc_daily",
  props: {
    "raw_data":{
  ...
  setup(props) {
    ...
    const init = (() => {
      chart.value = echarts.init(document.getElementById("MD_sc_daily"),'macarons')
      chart.value.setOption({
            title: {
              text: 'SC Throughput time-Daily',
              left: "center",
              textStyle: {
                fontSize: 14
              }
    ...
    const data_fresh = (mon) => {
      fetch_md_throughput_sc(query).then((res)=>{
        console.log(res)
