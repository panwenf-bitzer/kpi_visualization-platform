<!--
 * @FileDescription: child component for director alu throughput time daily
 * @Author: Pan,Wenfang
 * @Date: 2022/01/04
 ...
 * @LastEditTime: 2022/02/28
 -->
<template>
<div id="MD_alu_daily" :style="{height:height,width:width}" />
</template>
...
import {fetch_md_throughput_alu} from "../../api/Director_dashboard";
export default {
  name: "MD_alu_daily",
  props: {
    "raw_data":{
  ...
  setup(props) {
    ...
    const init = (() => {
      chart.value = echarts.init(document.getElementById("MD_alu_daily"),"macarons")
      chart.value.setOption({
            title: {
              text: 'ALU Throughput time-Daily',
              left: "center",
              textStyle: {
                fontSize: 14
              }
    ...
    const data_fresh = (mon) => {
      fetch_md_throughput_alu(query).then((res)=>{
        console.log(res)
