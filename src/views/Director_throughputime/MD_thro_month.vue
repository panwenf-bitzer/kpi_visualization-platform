<!--
 * @FileDescription: child component for director throughput time monthly
 * @Author: Pan,Wenfang
 * @Date: 2022/01/04
 ...
 * @LastEditTime: 2022/02/28
 -->
<template>
<div id="MD_thro_month" :style="{height:height,width:width}" />
</template>
...
import {fetch_md_throughput_all, fetch_md_throughput_alu} from "../../api/Director_dashboard";
...
export default {
  ...
  setup(props,context){
    ...
    const init = () => {
      chart.value = echarts.init(document.getElementById("MD_thro_month"),'macarons')
      chart.value.setOption(
          {
      title: {
        text: 'Throughput time_monthly',
        left: "center",
        textStyle: {
                fontSize: 14
              }
    ...
    const data_fresh=(()=> {
      fetch_md_throughput_all(query).then((res)=>{
        console.log(res)
