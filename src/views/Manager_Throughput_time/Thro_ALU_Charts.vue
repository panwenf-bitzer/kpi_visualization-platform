...
<!--              <el-date-picker v-model="search_item.date" value-format="YYYYMMDD" type="date" placeholder="选择日期"  class="handle-input"></el-date-picker>-->
            ...
            <h1 style="color: white;float: bottom;height: 60%">ALU Throughput time Dashboard</h1>
...
import {fetch_alu_throughput} from "../../api";
export default {
  name: "ALU_Charts",
  setup() {
      ...
      const Fetch_data=(()=>{
        ...
        fetch_alu_throughput(query_daily).then((res)=> {
          if (res.status===200){
      ...
      // });
        ...
        onActivated(()=>{
          refresh.value=setInterval(Fetch_data,3000)
