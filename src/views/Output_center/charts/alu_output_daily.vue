<!--
 * @FileDescription: child component for alu output
 * @Author: Pan,Wenfang
 * @Date: 2022/01/04
 ...
 * @LastEditTime: 2022/02/28
 -->
<template>
<div id="output_alu_daily" :style="{height:height,width:width}" />
</template>
...
import {fetch_daily_output_alu} from "../../../api/Output_center";
export default {
  name: "alu_output_daily",
  props: {
    "raw_data":{
      type:Object
      },
  ...
  setup(props) {
    ...
    const init = (() => {
      chart.value = echarts.init(document.getElementById("output_alu_daily"),"macarons")
      chart.value.setOption({
            title: {
              text: 'ALU output-Daily',
              left: "center",
              textStyle: {
                fontSize: 14
              }
    ...
    const data_fresh = (mon) => {
      fetch_daily_output_alu(query).then((res)=>{
        console.log(res)
