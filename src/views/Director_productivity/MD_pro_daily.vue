<!--
 * @FileDescription: child component for director daily productivity
 * @Author: Pan,Wenfang
 * @Date: 2022/01/04
 ...
 * @LastEditTime: 2022/02/28
 -->
<template>
<div id="MD_pro_daily" :style="{height:height,width:width}" />
</template>
...
import {fetch_md_productivity_cnc_assy} from "../../api/Director_dashboard";
export default {
  name: "MD_pro_daily",
  props: {
    "raw_data":{
  ...
  setup(props) {
    ...
    const init = (() => {
      chart.value = echarts.init(document.getElementById("MD_pro_daily"),"macarons")
      chart.value.setOption({
            title: {
              text: 'Total Productivity-Daily',
              left: "center",
              textStyle: {
                fontSize: 14
              }
    ...
    const data_fresh = (mon) => {
      fetch_md_productivity_cnc_assy(query).then((res)=>{
        console.log(res)
